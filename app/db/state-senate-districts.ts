...
export interface StateSenateDistrictStats {
  stsendist: number;
  latitude: number;
  longitude: number;
...
export async function getStateSenateDistricts(
  databaseUrl: string,
  signal: AbortSignal,
  { west, south, east, north }: BoundingBox
) {
  return withConnection(databaseUrl, async (sql) => {
    return await executeWithAbortSignal<StateSenateDistrictStats[]>(
      sql`
        SELECT
          stsendist,
          geom,
...
export async function* getStateSenateDistrictBuildings(
  databaseUrl: string,
  signal: AbortSignal,
  stsendist: number
) {
  ...
  try {
    const cursor = sql`
      ...
      WHERE pluto_latest_districts.stsen_dist = ${stsendist}::text
  `.cursor();
