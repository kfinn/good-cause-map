...
CREATE MATERIALIZED VIEW wow.gce_eligibility_nyad AS (
    SELECT
        ...
        nyad.geom AS geom,
        ST_X(ST_Centroid(nyad.geom)) :: double precision AS longitude,
        ST_Y(ST_Centroid(nyad.geom)) :: double precision AS latitude,
        ST_AsGeoJSON(nyad.geom) :: json as geom_json,
...
CREATE MATERIALIZED VIEW wow.gce_eligibility_nycc AS (
    SELECT
        ...
        nycc.geom AS geom,
        ST_X(ST_Centroid(nycc.geom)) :: double precision AS longitude,
        ST_Y(ST_Centroid(nycc.geom)) :: double precision AS latitude,
        ST_AsGeoJSON(nycc.geom) :: json as geom_json,
        COALESCE(SUM(unitsres), 0) :: integer AS unitsres,
        COALESCE(SUM(post_hstpa_rs_units), 0) :: integer AS post_hstpa_rs_units,
        COUNT(wow.gce_eligibility.*) :: integer AS bbls_count,
        MAX(co_issued) :: date AS max_co_issued,
...
CREATE MATERIALIZED VIEW wow.gce_eligibility_nyss AS (
    SELECT
        ...
        nyss.geom AS geom,
        ST_X(ST_Centroid(nyss.geom)) :: double precision AS longitude,
        ST_Y(ST_Centroid(nyss.geom)) :: double precision AS latitude,
        ST_AsGeoJSON(nyss.geom) :: json as geom_json,
        COALESCE(SUM(unitsres), 0) :: integer AS unitsres,
        COALESCE(SUM(post_hstpa_rs_units), 0) :: integer AS post_hstpa_rs_units,
        COUNT(wow.gce_eligibility.*) :: integer AS bbls_count,
        MAX(co_issued) :: date AS max_co_issued,
...
CREATE MATERIALIZED VIEW wow.gce_eligibility_nycg AS (
    SELECT
        ...
        nycg.geom AS geom,
        ST_X(ST_Centroid(nycg.geom)) :: double precision AS longitude,
        ST_Y(ST_Centroid(nycg.geom)) :: double precision AS latitude,
        ST_AsGeoJSON(nycg.geom) :: json as geom_json,
        COALESCE(SUM(unitsres), 0) :: integer AS unitsres,
        COALESCE(SUM(post_hstpa_rs_units), 0) :: integer AS post_hstpa_rs_units,
        COUNT(wow.gce_eligibility.*) :: integer AS bbls_count,
        MAX(co_issued) :: date AS max_co_issued,
